...
<div class="dropdown-cart-header">
    ...
    $count_row=$select_cart->num_rows;
    ?>
<span><?php echo$count_row?> Items</span>
<a href="cart.php">View Cart</a>
</div>

<ul class="shopping-list _num_cart">
    <?php
    $select_cart=$connect ->query("SELECT * FROM cart");
    $total=0;
    foreach ($select_cart as $select_cart){
        $id_pro_cart=$select_cart['id_product'];
        $select_id_product=$connect->query("SELECT * FROM product WHERE id='$id_pro_cart'")->fetch_assoc(); 

    $total+=$select_cart["count"]*$select_id_product["price"];

    ?>
<li class="cart-item">
<a href="javascript:void(0)" id_pro="<?php echo$select_id_product['id']?>" class="remove" title="Remove this item"><i class="lni lni-close "></i></a>
<div class="cart-img-head">
<a class="cart-img" href="product-details.php"><img src="admin/images/<?php echo$select_id_product['image']?>" alt="#"></a>
</div>
<div class="content">
<h4><a href="product-details.php">
<?php echo$select_id_product['name']?>"</a></h4>
<p class="quantity"><?php echo$select_cart["count"]?>x- <span class="amount"><?php echo$select_id_product['price']?>"</span></p>
</div>
</li>
<?php }?>
...
<div class="total">
<span>Total</span>
<span class="total-amount Total_price"><?php echo$total;?></span>
</div>
<div class="button">
...
<div class="row">
    <?php
    $select_product=$connect ->query("SELECT * FROM product");
    foreach ($select_product as $row_product) {
    ?>
<div class="col-lg-4 col-md-6 col-12">

<div class="single-category">
<h3 class="heading"><?php echo$row_product['category']?></h3>
<ul>
<li><a href="product-grids.php"><?php echo$row_product['name']?></a></li>
...
<div class="images">
<img src="admin/images/<?php echo$row_product['image']?>" width="150px"  alt="#">
</div>
...
<?php
// لعرض غدد الصفحات والمنتجات

if(isset($_GET['page'])){
        $num_page=($_GET['page']-1)*4;
        $select_product=$connect ->query("SELECT * FROM product LIMIT $num_page,4");
        }else{
            $select_product=$connect ->query("SELECT * FROM product LIMIT 0,4");
        }
...
<img src="admin/images/<?php echo$row_product['image']?>" height="180px" width="90px"  alt="#">
<div class="button">
<button  class="btn add_cart" id_product="<?php echo$row_product['id']?>"><i class="lni lni-cart"></i> Add to Cart</button>
</div>
</div>
 <div class="product-info">
<span class="category"><?php echo$row_product['category']?></span>
<h4 class="title">
<a href="product-grids.php"><?php echo$row_product['name']?></a>
</h4>
<ul class="review">
...
<div class="price">
<span>$<?php echo$row_product['price']?></span>
</div>
</div>
</div>
</div>
...
<?php
$select_product = $connect->query("SELECT * FROM product");
$pro_num = $select_product->num_rows;
$page =ceil($pro_num / 4);
?>
<div style="display: flex; justify-content: center;">
    <div class="text-center">
        <?php
        for ($i = 1; $i <= $page; $i++) {    
            echo "<a href='?page=$i' class='btn btn-primary m-1'>$i</a>";
        }
         ?>
    </div>
</div>
...
<script src="assets/js/jquery.com_jquery-3.7.0.min.js"></script>
...
<!-- Mirrored from demo.graygrids.com/themes/shopgrids/index.php by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 05 Dec 2022 23:35:56 GMT -->
    </html>
    <?php
    include "script.php";
    ?>
